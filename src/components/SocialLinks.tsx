...
function IconLink({
  ...
  return (
    <a
      ...
      aria-label={label}
      className={`flex h-10 w-10 items-center justify-center rounded-full text-white shadow-sm transition-transform hover:scale-105 ${colorClass}`}
    >
      {children}
    </a>
  );
...
export default function SocialLinks({ restaurant }: { restaurant: Restaurant }) {
  ...
  return (
    <div>
      <p className="mb-3 text-center font-display text-sm font-bold text-cream">تواصل معنا</p>
      <div className="flex flex-wrap items-center justify-center gap-3">
        {visible.map((l) => (
          <IconLink key={l.key} href={l.url} label={l.label} colorClass={l.colorClass} external>
            {l.icon}
          </IconLink>
        ))}
      </div>
